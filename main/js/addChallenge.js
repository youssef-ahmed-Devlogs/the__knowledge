...
const challengeDeails = document.getElementById("challenge-deails");
const challengeVideo = document.getElementById("challenge-video");
const challengePowerPointAnswer = document.getElementById(
  "challenge-power-point-answer"
);
const challengeDate = document.getElementById("challenge-date");
const challengeShowTime = document.getElementById("challenge-show-time");
const challengeAreaButtons = document.getElementsByName("challenge-area"); // Radio Buttons NOT NORMAL INPUT !
const challengeAdministrationConcerned = document.getElementById(
  "challenge-administration-concerned"
); // SELECT NOT INPUT !
const challengeBy = document.getElementById("challenge-by");
const evaluateSolutions = document.getElementById("evaluate-solutions");
const chiefArbitrator = document.getElementById("chief-arbitrator");
const juryMembers = document.getElementById("jury-members");
const termsAndConditions = document.getElementById("terms-and-conditions");
const challengeFinanceThereAnswer = document.getElementById(
  "challenge-finance-there-answer"
);
const challengeAundingAmount = document.getElementById(
  "challenge-funding-amount"
);
const challengeFinancingDetails = document.getElementById(
  "challenge-financing-details"
);
const isTherePrizeAnswer = document.getElementById("is-there-prize-answer");
const challengePrizeDetails = document.getElementById(
  "challenge-prize-details"
);

// Control Buttons Inputs is there ? [ toggle inputs ]
const challengePowerPoint = document.querySelectorAll(
  "#challenge-power-point span"
);
const challengeFinanceThere = document.querySelectorAll(
  "#challenge-finance-there span"
);
const isTherePrize = document.querySelectorAll("#is-there-prize span");

toggleInput(challengePowerPoint, challengePowerPointAnswer, "no", [
  challengeDate,
  challengeShowTime,
]);

changeButtonChoose(challengeFinanceThere, challengeFinanceThereAnswer);
challengeFinanceThere.forEach((btn) => {
  btn.addEventListener("click", () => {
    if (challengeFinanceThereAnswer.value.trim().toLowerCase() === "no") {
      challengeAundingAmount.parentElement.parentElement.style.display = "none";
      challengeAundingAmount.required = false;
    } else {
      challengeAundingAmount.parentElement.parentElement.style.display =
        "block";
      challengeAundingAmount.required = true;
    }

    if (challengeFinanceThereAnswer.value.trim().toLowerCase() === "no") {
      challengeFinancingDetails.parentElement.style.display = "none";
      challengeFinancingDetails.required = false;
    } else {
      challengeFinancingDetails.parentElement.style.display = "block";
      challengeFinancingDetails.required = true;
    }
  });
});

toggleInput(isTherePrize, isTherePrizeAnswer, "no", [challengePrizeDetails]);

// Character Limit Change for inputs
characterLimitChange(challengeTitle, 150);
characterLimitChange(challengeDeails, 5000);
characterLimitChange(evaluateSolutions, 3000);
characterLimitChange(termsAndConditions, 500);
characterLimitChange(challengeFinancingDetails, 500);
characterLimitChange(challengePrizeDetails, 500);

// Save to local Storge by formData object;
const formData = localStorage.getItem("formData")
  ? JSON.parse(localStorage.getItem("formData"))
  : {};

// All input in one array
const normalInputs = [
  challengeTitle,
  challengeDeails,
  challengeVideo,
  challengePowerPointAnswer,
  challengeBy,
  evaluateSolutions,
  chiefArbitrator,
  juryMembers,
  termsAndConditions,
  challengeFinanceThereAnswer,
  isTherePrizeAnswer,
];
...
challengeAreaButtons.forEach((radio) => {
...
challengeAdministrationConcerned.addEventListener("change", ({ target }) => {
  saveLocal(target.id, target.value);
});

if (formData[challengeAdministrationConcerned.id]) {
  challengeAdministrationConcerned.value =
    formData[challengeAdministrationConcerned.id];
}
  ...
  render() {
    ...
    this.shadow.innerHTML = `
      ...
      this.position
          ...
          <div class="header">
              <button type="button" class="prev-month" style="transform: rotate(
                -90deg);" aria-label="previous month"></button>
...
// Time Picker
$(function () {
  var input = $("#challenge-show-time").clockpicker({
    placement: "bottom",
    align: "left",
    autoclose: true,
    default: "now",
...
// FUNCTIONS
function toggleInput(buttons, varInput, word, toggleElements) {
  changeButtonChoose(buttons, varInput);
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      hideOrShowToggle(varInput, word, toggleElements);
    });
  });
}
...
function changeButtonChoose(buttons, varInput) {
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      switch (btn.getAttribute("data-challengeAnswer")) {
        case "no":
      ...
      varInput.value = btn.getAttribute("data-challengeAnswer");
    });
  });
}

function hideOrShowToggle(varInput, word, toggleElements) {
  if (varInput.value.trim().toLowerCase() === word) {
    toggleElements.forEach((toggleEle) => {
